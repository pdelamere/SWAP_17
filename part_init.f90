...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine load_Maxwellian(vth,Ni_tot_1,mass,mratio)
            use dimensions
            use boundary
            use inputs, only: PI, vsw, dx, dy, km_to_m, beta_particle, kboltz, mion, amp, grad, nf_init,b0_init,mu0,nx,cwpi
...
!add slow population           
            cnt = 0
            fNi = (float(nx)-200.0)/float(nx)
            do l = 0,nint(fNi*Ni_tot)
               flg = 0
               do while(flg .le. 0)
                  xp(l,1) = qx(1)+(1.0-pad_ranf())*(qx(nx-1)-qx(1))
                  f = 0.5*(1-tanh((xp(l,1)-(qx(400)-50*cwpi))/(10*cwpi)))+ 0.5*(1+tanh((xp(l,1)-(qx(400)+50*cwpi))/(10*cwpi)))
                  if (pad_ranf() .lt. f) flg = 1 
               enddo
               ...
               mrat(l) = mratio
                  ...
                  !f = (0.5*(1+tanh((xp(l,1)-qx(nx/2-100))/(10*dx)))+ 0.5*(1-tanh((xp(l,1)-qx(nx/2+100))/(10*dx))))-1.0
                  vth2=10.0+ (1-f)*30.0!va*sqrt(pl_beta(ijkp(l,1),ijkp(l,2),ijkp(l,3)))
                  ...
                  vp(l,1) = vx + 300.0+(1-f)*400.
                  vp(l,2) = vy +0.0*exp(-(xp(l,3)-qz(nz/2))**2/(20*dz_grid(nz/2))**2) !Gaussian velocity perturbation (20)
                  vp(l,3) = vz 
...
!add fast population
            do l = cnt,Ni_tot
               ...
               do while(flg .eq. 0)
                  xp(l,1) = qx(1)+(1.0-pad_ranf())*(qx(nx-1)-qx(1))
                  f = (0.5*(1+tanh((xp(l,1)-(qx(400)-50*cwpi))/(10*cwpi)))+ 0.5*(1-tanh((xp(l,1)-(qx(400)+50*cwpi))/(10*cwpi))))-1.0
                  if (pad_ranf() .le. f) flg = 1 
               enddo
                  ...
                  beta_p(l) = 0.25*beta_particle

                  call get_pindex(i,j,k,l)

                  vth2= 10 + f*30.0!va*sqrt(pl_beta(ijkp(l,1),ijkp(l,2),ijkp(l,3)))
